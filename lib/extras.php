...
function show_custom_embroidery_colour_on_cart_and_checkout( $cart_data, $cart_item ) {
  ...
  if ( isset( $cart_item['custom-embroidery-colour'] ) && $custom_field_value = $cart_item['custom-embroidery-colour'] ) {
    $custom_items[] = array(
...
function show_custom_embroidery_text_on_cart_and_checkout( $cart_data, $cart_item ) {
  ...
  if ( isset( $cart_item['custom-embroidery-text'] ) && $custom_field_value = $cart_item['custom-embroidery-text'] ) {
    $custom_items[] = array(
...
function set_product_prices_admin_page() {
  ...
  if ( ! isset( $_POST['submit'] ) ) {
    ...
    <form method="post" action="<?php echo basename( get_permalink() ); ?>">
      ...
      <span><label for="setprice-exclusions">Post IDs to exclude seperated by commas</label><input
          id="setprice-exclusions" type="text" name="exclusions"></span><br/>
...
add_action( 'init', __NAMESPACE__ . '\\setting_user_cookie' );

function setting_user_cookie() {
  if ( is_user_logged_in() ) {
    if ( ! isset( $_COOKIE['dt_user_cookie'] ) ) {
      $time = get_user_meta( get_current_user_id(), 'dt_user_cookie', true );
      if ( empty( $user_dt_user_cookie ) ) {
        $time = microtime( true );
        update_user_meta( get_current_user_id(), 'dt_user_cookie', $time );
      }
      setcookie( 'dt_user_cookie', $time, time() + 365 * DAY_IN_SECONDS, COOKIEPATH,
        COOKIE_DOMAIN, is_ssl() );
    }
  } else {
    if ( ! isset( $_COOKIE['dt_user_cookie'] ) ) {
      setcookie( 'dt_user_cookie', microtime( true ), time() + 365 * DAY_IN_SECONDS, COOKIEPATH,
        COOKIE_DOMAIN, is_ssl() );
    }
  }
}

add_action( 'user_register', __NAMESPACE__ . '\\dtcookie_registration_save', 10, 1 );

function dtcookie_registration_save( $user_id ) {
  if ( isset( $_COOKIE['dt_user_cookie'] ) ) {
    update_user_meta( $user_id, 'dt_user_cookie', $_COOKIE['dt_user_cookie'] );
  }
}


add_action( 'wp_ajax_customiser_validify', __NAMESPACE__ . '\\customiser_capture' );
add_action( 'wp_ajax_nopriv_customiser_validify', __NAMESPACE__ . '\\customiser_capture' );
function customiser_capture() {
  if ( isset( $_POST['data'] ) && ! empty( $_POST['data'] ) ) {
    global $wpdb;
    $captured = $_POST['data'];
    if ( ! isset( $captured['text'] ) || empty( $captured['text'] ) ) {
      wp_die();
    }
    $text              = $captured['text'];
    $embroidery_colour = isset( $captured['text_colour'] ) ? $captured['text_colour'] : '';
    $product_type      = isset( $captured['product_type'] ) ? $captured['product_type'] : '';
    $product_colour    = isset( $captured['product_colour'] ) ? $captured['product_colour'] : '';
    $timestamp         = current_time( 'mysql' );
    $user_identifier   = isset( $captured['user_identifier'] ) ? $captured['user_identifier'] : '';
    $session_id        = isset( $captured['embroidery_session'] ) ? $captured['embroidery_session'] : '';
    $wpdb->insert( 'custom_embroidery_captured', [
      'custom_embroidery_text' => $text,
      'timestamp'              => $timestamp,
      'product_type'           => $product_type,
      'product_colour'         => $product_colour,
      'embroidery_colour'      => $embroidery_colour,
      'user_identifier'        => $user_identifier,
      'embroidery_session'             => $session_id
    ] );
  }
